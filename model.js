...
sensorBody: {
  ...
  name: ["scent_unit_body",
  "scent_unit_body001"],
  map: textureLoader.load(`$ {
      hostURL
    }

    /texture/Vr_sensor.webp`),
  color: "#535353",
  click: true,
...
info_modal.querySelectorAll(".modal_data").forEach((e, i)=> {
  e.dataset.id=="arena"
  ? e.classList.add("show") : e.classList.remove("show");
});
...
//// load model
function loadModel() {
new GLTFLoader().load( // resource URL
...
var heaterLight1=new THREE.Mesh(new THREE.CylinderGeometry(0.05,
0.05,
7.2,
10, 5),
new THREE.MeshBasicMaterial({
...
//room light
const roomLight=new THREE.HemisphereLight(model.ambient.color,
model.shadow.color,
...
labelDiv.style.cssText=` background-color: $ {
model.labels.areaBackground
}

;

font-size: $ {
model.labels.areaFontSize
}
...
color: $ {
model.labels.pulseDot
}

;

text-align: $ {
model.labels.alignment
}
...
//add fan box
let fanCube=new THREE.Mesh( // new THREE.BoxGeometry(1.4, 2, 2),
